...
digraph process {
    ...
    "Fix loop: round 1 resume, round 2 fresh+tier-up; cap 2" [shape=box];
    "Still open at cap -> adjudicate (park / BLOCKED)" [shape=box];
    "Ledger: Task N complete" [shape=box];
    "More tasks?" [shape=diamond];
    "Final whole-branch review (most-capable), one fix wave" [shape=box];
    "rm workspace; finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Findings?" -> "Fix loop: round 1 resume, round 2 fresh+tier-up; cap 2" [label="yes"];
    "Findings?" -> "Ledger: Task N complete" [label="no"];
    "Fix loop: round 1 resume, round 2 fresh+tier-up; cap 2" -> "Still open at cap -> adjudicate (park / BLOCKED)";
    "Still open at cap -> adjudicate (park / BLOCKED)" -> "Ledger: Task N complete";
    ...
    "Final whole-branch review (most-capable), one fix wave" -> "rm workspace; finishing-a-development-branch";
}
```
